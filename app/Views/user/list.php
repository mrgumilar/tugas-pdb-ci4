...
<hr>
<table id="listuser" class="table table-striped dt-responsive "
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center"><img onclick="gambar('<?= $data['user_id'] ?>')"
                    src="<?= base_url('img/user/thumb/' . 'thumb_' . $data['foto']) ?>" width="120px"
            ...
            <?php if ($data['level'] != 'admin') { ?>
                ...
                <button type="button" onclick="toggle('<?= $data['user_id'] ?>')"
                    class="btn btn-circle btn-sm <?= $data['active'] ? 'btn-secondary' : 'btn-success' ?>"
                    title="<?= $data['active'] ? 'Nonaktifkan' : 'Aktifkan' ?>"><i class="fa fa-fw fa-power-off"></i>
                </button>
                <?php } ?>
            </td>
        </tr>
